...
DbManager::DbManager() {
    ...
    m_db.setDatabaseName("/home/jonathan/Desktop/Jonathan/GUI/DB/teste.db");
    if (!m_db.open()) {
...
void DbManager::insertSession() {
    ...
    if (query.exec()) {
        while (query.next()) {
            ...
            if (id_sesion == 1) {
                query.prepare("insert into def_settings(serial_number, language0, output, standby) values (" + QString::number(can -> getRunSerialNum()) + ", \"English\", 120, 0)");
                if (query.exec()) {
                } else {
                    qDebug() << "def_settings error:" << query.lastError();
                }
                query.prepare("insert into settings(def_serial_number, language0, output, standby) values (" + QString::number(can -> getRunSerialNum()) + ", \"English\", 120, 0)");
                if (query.exec()) {
                } else {
...
void DbManager::defSettings()
{
    QSqlQuery query(m_db);
    query.prepare("delete from settings where def_serial_number = " + QString::number(serialNumber));
    if (query.exec()){
    }
    else{
        qDebug() << "Delete settings error: " << query.lastError();
    }

    query.prepare("insert into settings select * from def_settings");
    if (query.exec()){
    }
    else{
        qDebug() << "Default settings error: " << query.lastError();
    }
    selectLanguage();
    selectOutput();
    selectStandby();
...
void DbManager::selectLanguage()
{
    QSqlQuery query(m_db);
    query.prepare("select language0 from settings");
    if (query.exec()) {
        while (query.next()) {
            language = query.value(0).toString();
        }
    } else {
        qDebug() << "language error: " << query.lastError();
    }
}
